...
export default function RankingSequential({ items, onRankingComplete, currentRankings }: RankingSequentialProps) {
  const [selections, setSelections] = useState<string[]>(() => {
    if (currentRankings && Object.keys(currentRankings).length === items.length) {
      return [...Object.entries(currentRankings)].sort(([, a], [, b]) => a - b).map(([id]) => id);
    }
    return [];
  });
  ...
  const remainingItems = items.filter(i => !selections.includes(i.key));

  const handleSelect = (itemKey: string) => {
    if (selections.includes(itemKey)) return;
    const newSelections = [...selections, itemKey];
    setSelections(newSelections);
    if (newSelections.length === items.length) {
      const rankings: Record<string, number> = {};
      newSelections.forEach((id, idx) => { rankings[id] = idx + 1; });
    ...
    <div className="space-y-3">
      ...
      {selections.length > 0 && (
        <div className="space-y-1 mb-2">
          {selections.map((key, idx) => {
            const item = items.find(i => i.key === key);
            if (!item) return null;
            return (
              <div key={key} className="flex items-center gap-3 p-2 rounded-lg bg-accent/10 border border-accent/30">
                <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold bg-accent text-accent-foreground shrink-0">{idx + 1}</span>
          ...
          {!isComplete && (
            <button onClick={handleUndo} className="text-xs text-muted-foreground hover:text-foreground underline">Undo last selection</button>
          )}
        </div>
      )}
        ...
        <div className="space-y-1">
          {remainingItems.map((item) => (
            <button key={item.key} onClick={() => handleSelect(item.key)}
              ...
              <span className="w-6 h-6 rounded-full flex items-center justify-center text-xs border-2 border-dashed border-muted-foreground/30 text-muted-foreground shrink-0">{currentStep + 1}</span>
